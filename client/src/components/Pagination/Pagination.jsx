...
export const Pagination = ({ pagina, setPagina, maximo }) => {
    ...
    const totalPages = maximo; // Total de páginas

    const nextPage = () => {
        if (pagina < totalPages) {
            setPagina(pagina + 1); // Actualiza la página actual en el estado
        }
    };
    ...
    const goToFirstPage = () => {
        setPagina(1); // Ir a la primera página
    };

    const goToLastPage = () => {
        setPagina(totalPages); // Ir a la última página
    };
    ...
    return (
        <div className={styles.container}>
            ...
            <span className={styles.pageNumber}>{pagina}</span>
